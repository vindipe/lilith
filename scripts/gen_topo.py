...
def create_scale_free_topology(df, mode):    
    ...
    plt.title(f'Nodes degree in Scale-free')
    ...
    plt.savefig(f"results/img/degree_scale_free_1_{mode}.png")
    plt.savefig(f'results/img/degree_scale_free_1_{mode}.pdf', dpi=500, bbox_inches='tight')    
...
def create_torus_topology(df, mode):
    ...
    grid = [[(i, j) for j in range(5)] for i in range(2)]    
...
def check_bandwidth(value):
    ...
    if value > 10 and value < 1:
        raise argparse.ArgumentTypeError("The bandwidth value must be between 1 and 10.")
    return value

def check_type(value):
    if value not in ["full-mesh", "scale-free-t", "scale-free-l", "scale-free-r", "double-l", "double-s", "torus-l", "torus-t", "torus-r", "fat-tree-t", "fat-tree-l", "hypercube"]:
        raise argparse.ArgumentTypeError('Accepted values are: "full-mesh", "scale-free-t", "scale-free-l", "scale-free-r", "double-l", "double-s", "torus-l", "torus-t", "torus-r", "fat-tree-t", "fat-tree-l", "hypercube"')
    return value
...
def check_dynamic(value):
    ...
    if value not in [0, 1, 2, 3, 4, 5]:
        raise argparse.ArgumentTypeError("The dynamic value must be either 0, 1, 2, 3, 4, or 5.")
    return value
...
parser.add_argument('--bandwidth', type=check_bandwidth, default=1, help='Bandwidth value (integer between 1 and 10)')
parser.add_argument('--type', type=check_type, default='full-mesh', help="Type value ('full-mesh', 'double-l', 'double-s', 'scale-free-t', 'scale-free-l', 'scale-free-r', 'torus-l', 'torus-r', 'torus-t', 'fat-tree-t', 'fat-tree-l', 'hypercube').")
...
parser.add_argument('--dynamic', type=check_dynamic, default='0', help="Dynamic value (either 0:no, 1:packet_drop, 2:congestion, 3:switch-leave, 4:crash, 5:latency-evolution).")
...
if 'full-mesh' in mode:
    aws_data = create_full_mesh_topology(aws_data)
elif 'scale-free' in mode:
    aws_data = create_scale_free_topology(aws_data, mode)
    # in create_scale_free_topology uso unique_region. se dopo qui ri-uso devo sistemare
elif 'torus' in mode:
    aws_data = create_torus_topology(aws_data, mode)
elif 'fat-tree' in mode:
    first_gate_regions, last_gate_regions, aws_data = create_fat_tree_topology(aws_data, mode)
    ...
    aws_data = create_hypercube_topology(aws_data)     
# print()
# print("AWS : after")
# print(aws_data.shape[0])
...
# regions_occurrences = {}
# sum_throughput = 0
df_for_dynamic_latencies = pd.DataFrame(columns=['origin', 'dest', 'switch'])

for index, row in aws_data.iterrows():
    ...
    # upload_bandwidth = bw_data[bw_data['region'].str.contains(origin)]['bandwidth'].iloc[0]
    # upload_bandwidth = bw_data[bw_data['region'].apply(lambda x: origin in x)].sort_values(by='region', key=lambda x: -len(x))['bandwidth'].iloc[0]
    ...
    # upload_bandwidth = int(upload_bandwidth * 1024)
    ...
    # download_bandwidth = int(download_bandwidth * 1024)
    
    # link = ET.SubElement(root.find(".//links"), "link") 
    # link.set("origin", f'{origin}')
    # link.set("dest", f'{dest}')
    ...
    for hop in range(hops + 1):
        ...
        if hop == 0:
            link.set("origin", f'{origin}')
            link.set("dest", f's{bridge_used+1}')
            # link.set("latency", f"{latency/2}")
            link.set("latency", f"{latency}")
                            
            bridge_used += 1
            
            if dynamic == 5:               
                # Creazione della riga per il DataFrame
                new_row = {'origin': origin, 'dest': dest, 'switch': f's{bridge_used}'}
                
                # Aggiunta della riga al DataFrame
                df_for_dynamic_latencies = pd.concat([df_for_dynamic_latencies, pd.DataFrame([new_row])], ignore_index=True)
        elif hop == hops:
            link.set("origin", f's{bridge_used}')
            link.set("dest", f'{dest}')
        ...
        link.set("network", "kollaps_network")
        link.tail = "\n"
# print("Sum throughput : ", sum_throughput)

for i in range(bridge_used):
    bridge = ET.SubElement(root.find(".//bridges"), "bridge", name=f's{i+1}')
    bridge.tail = '\n'
...
if dynamic > 0:   
    ...
    dynamic_regions = int(len(regions) * percentage)   # 30% of regions
    random.seed(443)
    random_regions_selection = random.sample(regions, dynamic_regions)
    # print(random_regions_selection)
    
    # Random selection of nodes
    dynamic_nodes = int(len(regions) * nodes_per_region * percentage)   # 30% of nodes
    nodes = root.findall('.//services/service')
    # exclude "dashboard" or "primary" or "secondary"
    nodes = [node for node in nodes if not re.search(r'dashboard|primary|secondary', node.attrib['name'])]
    random.seed(443)
    selected_nodes = random.sample(nodes, dynamic_nodes)   
    ...
    random.seed(443)
    selected_links = random.sample(links, int(len(links) * percentage))    # 30% of links
        
    #deploy time + workload_prep_time + workload_time/3 . 
    if blockchain == "algorand": #(11*60=~700)+(100)+(276/3=100)=950  
        # dynamic_start = 1200.0
        dynamic_start = 650.0 
        # dynamic_start = 600.0  + 60.0
    elif blockchain == "poa": #(7*60=~500)+(100)+(276/3=140)=7500
        # dynamic_start = 1000.0
        dynamic_start = 540.0
        # dynamic_start = 540.0 + 60.0
    elif blockchain == "quorum": #(7*60=~500)+(100)+(276/3=140)=750 
        # dynamic_start = 1100.0 
        dynamic_start = 600.0 
        # dynamic_start = 600.0  + 60.0
    elif blockchain == "diem": #(38*60=~2300)+(100)+(276/3=140)=2650
        dynamic_start = 3300.0      #NOW
        # dynamic_start = 2650.0  #PREV
        # dynamic_start = 2650.0 + 60.0
    elif blockchain == "solana": #(30/35*60=~2100)+(100)+(276/3=140)=2550
        dynamic_start = 2530.0
        # dynamic_start = 2150.0
        # dynamic_start = 2150.0 + 60.0
        
    # nodes re-join (if crash activated) 
    ...
    if dynamic == 1: #packet_drop        
        ...
        for link in selected_links:
            origin=link.attrib['origin']
            dest=link.attrib['dest']
            schedule = ET.SubElement(root.find(".//dynamic"), "schedule", origin=f'{origin}', dest=f'{dest}', time=f'{dynamic_start}', upload=f'{int(upload_bandwidth * percentage)}Mbps', download=f'{int(download_bandwidth * percentage)}Mbps')
            schedule.tail = '\n'          
        ...
        #         if elem.get('origin') == region:
        #             dest = elem.get('dest')              
        #             schedule = ET.SubElement(root.find(".//dynamic"), "schedule", origin=f'{region}', dest=f'{dest}', time=f'{dynamic_start}', upload=f'{int(upload_bandwidth * percentage)}Mbps', download=f'{int(download_bandwidth * percentage)}Mbps')
        #             schedule.tail = '\n'            
        ...
        for node in selected_nodes:      
            ...
            print(node_name)
            schedule = ET.SubElement(root.find(".//dynamic"), "schedule", name=f'{node_name}', time=f'{dynamic_start}', action='crash')
            # schedule = ET.SubElement(root.find(".//dynamic"), "schedule", name=f'{region}', time=f'{dynamic_start}', action='leave')
            schedule.tail = '\n'

            # schedule = ET.SubElement(root.find(".//dynamic"), "schedule", name=f'{region}-n{i+1}', time=f'{join_time_after_crash}', action='join')
            # schedule.tail = '\n'
    elif dynamic == 5: #real-world latency           
        #deploy time + workload_prep_time + workload_time/3 . ANNULLO-CHECK-LATENCY
        if blockchain == "algorand": #(11*60=~700)+(100)+(276/3=100)=950  
            # dynamic_start = 1100.0  #Paypal
            # dynamic_start = 1200.0  #Gafam
            dynamic_start = 600.0  #Gafam-short
        elif blockchain == "poa": #(7*60=~500)+(100)+(276/3=140)=7500
            # dynamic_start = 1100.0  #Paypal
            # dynamic_start = 1200.0  #Gafam
            dynamic_start = 540.0  #Gafam-short
        elif blockchain == "quorum": #(7*60=~500)+(100)+(276/3=140)=750 
            # dynamic_start = 1100.0  #Paypal
            # dynamic_start = 1200.0  #Gafam
            dynamic_start = 600.0  #Gafam-short
        elif blockchain == "diem": #(38*60=~2300)+(100)+(276/3=140)=2650
            # dynamic_start = 3000.0  #Paypal
            # dynamic_start = 3150.0  #Gafam
            dynamic_start = 2250.0  #Gafam-short
        elif blockchain == "solana": #(30/35*60=~2100)+(100)+(276/3=140)=2550
            # dynamic_start = 2650.0  #Paypal
            # dynamic_start = 2800.0  #Gafam
            dynamic_start = 2150.0  #Gafam-short
         
        # join_time_after_crash = dynamic_start + 300.0
        ping_data = pd.read_csv("misc/cloudping/ping.csv")
        regions = ['af-south-1', 'ap-northeast-1', 'ap-south-1', 'ap-southeast-2', 'eu-north-1', 'eu-south-1', 'me-south-1', 'sa-east-1', 'us-east-2', 'us-west-2']
        # regions = [abbreviate_regions(region) for region in regions]
        for region1, region2 in itertools.combinations(regions, 2):            
            ...
            if region1 != region2:            
                # I filter the data based on the pair of regions.
                filtered_data = ping_data[((ping_data['src_region'] == region1) & (ping_data['dst_region'] == region2)) |
                                        ((ping_data['src_region'] == region2) & (ping_data['dst_region'] == region1))]            
                # print(filtered_data)
                
                region1 = abbreviate_regions(region1)
                region2 = abbreviate_regions(region2)
                
                if not filtered_data.empty:
                    # Find min and max values
                    min_ping = int(filtered_data['ping_avg'].min())
                    max_ping = int(filtered_data['ping_avg'].max())
                    # print(max_ping)
                    max_ping = max_ping * 1
                    
                    ping_difference = max_ping - min_ping
                    interval_time = 200 #60*3 minutes of increasing latencies
                    # interval_time = 60 #1 minute for GAFAM-short
                    ping_increase = ping_difference/interval_time
                    ...
                    aws_data_filtered = aws_data[((aws_data['src_region'] == region1) & (aws_data['dst_region'] == region2)) | ((aws_data['src_region'] == region2) & (aws_data['dst_region'] == region1))] 
                    # print(aws_data_filtered)
                    origin = aws_data_filtered['src_region'].iloc[0]
                    region2 = aws_data_filtered['dst_region'].iloc[0]
                    # QUI imposto la latenza minima dal mio dataset AWS
                    # min_ping = aws_data_filtered['latency'].iloc[0]
                    
                    dest = df_for_dynamic_latencies.loc[(df_for_dynamic_latencies['origin'] == origin) & (df_for_dynamic_latencies['dest'] == region2), 'switch'].values[0]
                    
                    i=0
                    tmp_dynamic_start = dynamic_start
                    tmp_min_ping = min_ping
                    
                    schedule = ET.SubElement(root.find(".//dynamic"), "schedule", origin=f'{origin}', dest=f'{dest}', time='1.0', latency=f'{min_ping}')
                    schedule.tail = '\n'
                    
                    while i <= interval_time:                        
                        schedule = ET.SubElement(root.find(".//dynamic"), "schedule", origin=f'{origin}', dest=f'{dest}', time=f'{dynamic_start}', latency=f'{min_ping}')
                        schedule.tail = '\n'        
                                            
                        i += 1
                        dynamic_start += 1                    
                        min_ping += ping_increase
                        min_ping = round(min_ping, 1)                    
                        
                    # schedule = ET.SubElement(root.find(".//dynamic"), "schedule", origin=f'{origin}', dest=f'{dest}', time=f'{join_time_after_crash}', latency=f'{min_ping}')
                    schedule = ET.SubElement(root.find(".//dynamic"), "schedule", origin=f'{origin}', dest=f'{dest}', time=f'{dynamic_start+60}', latency=f'{tmp_min_ping}')    # 60 secondi dove ho max latenza running
                    # schedule = ET.SubElement(root.find(".//dynamic"), "schedule", origin=f'{origin}', dest=f'{dest}', time=f'{dynamic_start}', latency=f'{tmp_min_ping}')    # 60 secondi dove ho max latenza running
                    schedule.tail = '\n'                       
                    
                    dynamic_start = tmp_dynamic_start
...
avg_latency = aws_data['latency'].mean()
print("Avg latency:", avg_latency)
std_latency = aws_data['latency'].std()
print("Std latency:", std_latency)
avg_throughput = aws_data['throughput'].mean()
print("Avg throughput:", avg_throughput)
std_throughput = aws_data['throughput'].std()
print("Std throughput:", std_throughput)
if dataset == 'our':
    avg_hops = aws_data['hops'].mean()
    print("Avg hops:", avg_hops)


# Topo-Plot context-------------------------------------------
# aws_data['src_region'] = aws_data['src_region'].apply(region_fix)
# aws_data['dst_region'] = aws_data['dst_region'].apply(region_fix)
# for region in regions:
#     region_fix(region)

# aws_data = aws_data.sort_values(by='latency')

# Graph creation
G = nx.from_pandas_edgelist(aws_data, source='src_region', target='dst_region', edge_attr=True, create_using=nx.DiGraph)
...
edge_labels = {(src, dst): {'latency': aws_data[(aws_data['src_region'] == src) & (aws_data['dst_region'] == dst)]['latency'].values[0],
                            'throughput': aws_data[(aws_data['src_region'] == src) & (aws_data['dst_region'] == dst)]['throughput'].values[0],
                            'hops': aws_data[(aws_data['src_region'] == src) & (aws_data['dst_region'] == dst)]['hops'].values[0]}
...
plt.axis('off')
plt.savefig(f'results/img/{mode}_network_graph.png', format='png', bbox_inches='tight')
plt.savefig(f'results/img/{mode}_network_graph.pdf', dpi=500, bbox_inches='tight')


edges = list(zip(aws_data['src_region'], aws_data['dst_region']))
G = nx.Graph()
...
edge_width = 4
# Node draw
nx.draw_networkx_nodes(G, pos, node_size=15000, node_color='#DCDCDC', alpha=0.9)  # Grigio molto chiaro
# Labels draw
nx.draw_networkx_labels(G, pos, font_color='black', font_size=23)  # Font nero e ingrandito
nx.draw_networkx_edges(G, pos, width=edge_width, alpha=0.7, edge_color='black', arrows=True, arrowstyle='-|>')
...
plt.axis('off')
plt.tight_layout()      
plt.savefig(f"results/img/{mode}_network_graph_2.png")
plt.savefig(f'results/img/{mode}_network_graph_2.pdf', dpi=500, bbox_inches='tight')        